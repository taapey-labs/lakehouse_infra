...
- Dedicated Route 53 private hosted zones (not a `cloud.databricks.com` apex):
  ...
  - `oregon.privatelink.cloud.databricks.com` → **REST** VPCE (control-plane region name; parameter `ControlPlanePrivatelinkDnsName`)
- Control-plane region: an **N. California** workspace's control plane runs in **Oregon** (`oregon.cloud.databricks.com`, `44.234.192.32/28`), reached over PrivateLink as `oregon.privatelink.cloud.databricks.com`. Databricks requires **all** workspace/control-plane URLs to resolve to the REST/General Access VPC endpoint, so this hostname is pinned to the REST VPCE. Without it, classic compute reaches the control plane on **public** IPs and Unity Catalog returns **KCUC4** `Unauthorized network access to workspace`. Set `ControlPlanePrivatelinkDnsName` to match your control-plane region (see the [region/endpoint doc](https://docs.databricks.com/aws/en/resources/ip-domain-region)).
